...
// Methods
    ...
    internal abstract void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff);
    ...
    internal static string GetRelativeAddressOfNodesetXmlReadable(XmlDiffNode firstNode, XmlDiffNode lastNode)
    {
        Debug.Assert(!(firstNode is XmlDiffAttributeOrNamespace) &&
                      !(lastNode is XmlDiffAttributeOrNamespace));
        ...
        if (bInterval)
        {
            switch (lastNode.NodeType)
            {
                case XmlDiffNodeType.Attribute :
                    sb.Append(((Microsoft.XmlDiffPatch.XmlDiffAttribute)(lastNode)).LocalName);
                    break;
                case XmlDiffNodeType.Element :
                    sb.Append(((Microsoft.XmlDiffPatch.XmlDiffElement)(lastNode)).LocalName);
                    break;
                default :
                    sb.Append(lastNode.Position);
                    break;
    ...
    internal static void GetAddressOfAttributeIntervalXmlReadable(AttributeInterval interval, XmlWriter xmlWriter)
    {
        Debug.Assert(interval != null);
        if (interval._next == null)
        {
            if (interval._firstAttr == interval._lastAttr)
            {
                xmlWriter.WriteAttributeString("match", ((XmlDiffAttribute)interval._firstAttr).LocalName);
                return;
            }
    ...
    internal static void WriteAbsoluteMatchAttributeXmlReadable(XmlDiffNode node, XmlWriter xmlWriter)
    {
        XmlDiffAttribute attr = node as XmlDiffAttribute;
        ...
        {
            case XmlDiffNodeType.Attribute:
                xmlWriter.WriteAttributeString("match", ((XmlDiffAttribute)node).LocalName);
                break;
            case XmlDiffNodeType.Element:
                xmlWriter.WriteAttributeString("match", ((XmlDiffElement)node).LocalName);
                break;
            case XmlDiffNodeType.ShrankNode :
                ((XmlDiffShrankNode)node).WriteTo(xmlWriter);
                break;
            default:
                xmlWriter.WriteAttributeString("match", node.GetAbsoluteAddress());
                break;
...
// Methods
    ...
    internal void WriteChildrenToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        DiffgramOperation curOp = _firstChildOp;
        while (curOp != null)
        {
            curOp.WriteToXmlReadable(xmlWriter, xmlDiff);
            curOp = curOp._nextSiblingOp;
        }
    }
...
// Methods
    ...
    internal override void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        xmlWriter.WriteStartElement("", "add", XmlDiff.NamespaceUri);
        ...
        switch (_targetNode.NodeType)
        {
            case XmlDiffNodeType.Element:
                {
                    XmlDiffElement el = _targetNode as XmlDiffElement;
                    xmlWriter.WriteRaw(el.OuterXml);
                    break;
                }
            case XmlDiffNodeType.Attribute:
                {
                    ...
                    if ( at.Prefix != string.Empty )
                        xmlWriter.WriteAttributeString("prefix", at.Prefix);
                    xmlWriter.WriteAttributeString("attr", at.LocalName);
                    xmlWriter.WriteAttributeString("value", at.Value);
                    break;
                }
            case XmlDiffNodeType.Namespace:
                {
                    ...
                    if (ns.Prefix != string.Empty)
                    {
                        ...
                        xmlWriter.WriteAttributeString("name", "xmlns");
                    xmlWriter.WriteAttributeString("namespace", ns.NamespaceURI);
                    break;
                }
            case XmlDiffNodeType.CDATA:
                {
...
// Methods
    ...
    {
        xmlWriter.WriteStartElement("", "add", XmlDiff.NamespaceUri);
        WriteAbsoluteMatchAttributeXmlReadable(_sourceNode, xmlWriter);
        WriteChildrenToXmlReadable(xmlWriter, xmlDiff);
        xmlWriter.WriteEndElement();
    }
...
// Methods
    ...
    internal override void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        xmlWriter.WriteStartElement("", "remove", XmlDiff.NamespaceUri);

        if (_sourceNode.NodeType == XmlDiffNodeType.Attribute)
        {
            xmlWriter.WriteAttributeString("attr", ((XmlDiffAttribute)_sourceNode).LocalName);
            WriteChildrenToXmlReadable(xmlWriter, xmlDiff);
        }
        else
        {
                xmlWriter.WriteRaw(_sourceNode.OuterXml);
        }
        xmlWriter.WriteEndElement();
    }
...
// Methods
    ...
    internal override void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        ...
        {
            xmlWriter.WriteStartElement("", "remove", XmlDiff.NamespaceUri);
            xmlWriter.WriteRaw(_firstSourceNode.OuterXml);
            xmlWriter.WriteEndElement();
        }
        else
        {
            XmlDiffNode curNode = _firstSourceNode;
            while ( curNode != null)
            {
                xmlWriter.WriteStartElement("", "remove", XmlDiff.NamespaceUri);
                xmlWriter.WriteRaw(curNode.OuterXml);
                xmlWriter.WriteEndElement();
                if (curNode == _lastSourceNode)
                    break; // we are at the last node to remove, we do not want to remove the following nodes in the source
                curNode = curNode._nextSibling;
            }
        }    
...
// Methods
    ...
    {
        xmlWriter.WriteStartElement("", "remove", XmlDiff.NamespaceUri);
        GetAddressOfAttributeIntervalXmlReadable(_attributes, xmlWriter);
        Debug.Assert(_operationID == 0);
        xmlWriter.WriteEndElement();
    }
...
// Methods
    ...
    internal override void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        switch (_op)
        {
            case XmlDiffOperation.ChangeAttr:
                {
                    xmlWriter.WriteStartElement("", "change", XmlDiff.NamespaceUri);
                    xmlWriter.WriteAttributeString("attr", _sourceNode.GetRelativeAddress().Substring(1));
                    XmlDiffAttribute sourceAttr = (XmlDiffAttribute)_sourceNode;
                    XmlDiffAttribute targetAttr = (XmlDiffAttribute)_targetNode;
                    if (sourceAttr.Prefix != targetAttr.Prefix && !xmlDiff.IgnorePrefixes && !xmlDiff.IgnoreNamespaces)
                        xmlWriter.WriteAttributeString("prefix", targetAttr.Prefix);
                    if (sourceAttr.NamespaceURI != targetAttr.NamespaceURI && !xmlDiff.IgnoreNamespaces)
                        xmlWriter.WriteAttributeString("ns", targetAttr.NamespaceURI);
                    xmlWriter.WriteAttributeString("value", targetAttr.Value);
                    xmlWriter.WriteEndElement();
                    break;
                }
            case XmlDiffOperation.ChangeElementName:
                {
                    ...
                    if (sourceEl.OuterXml.IndexOf(">") + 1 >= sourceEl.OuterXml.Length)
                        balise = sourceEl.OuterXml.Substring(0, sourceEl.OuterXml.IndexOf('/')) + '>';
                    else
                        balise = sourceEl.OuterXml.Substring(0, sourceEl.OuterXml.IndexOf('>') + 1);

                    xmlWriter.WriteRaw(balise);
                    xmlWriter.WriteStartElement("", "change", XmlDiff.NamespaceUri);
                    if (sourceEl.LocalName != targetEl.LocalName)
                        xmlWriter.WriteAttributeString("EltName", targetEl.LocalName);
                    if (sourceEl.Prefix != targetEl.Prefix && !xmlDiff.IgnorePrefixes && !xmlDiff.IgnoreNamespaces)
                        xmlWriter.WriteAttributeString("prefix", targetEl.Prefix);
                    if (sourceEl.NamespaceURI != targetEl.NamespaceURI && !xmlDiff.IgnoreNamespaces)
                        xmlWriter.WriteAttributeString("ns", targetEl.NamespaceURI);
                    ...
                    if (balise.IndexOf(" ") > 0)
                        xmlWriter.WriteRaw("</" + balise.Substring(1, balise.IndexOf(" ")) + ">");
                    else
                        xmlWriter.WriteRaw("</" + balise.Substring(1));
            ...
            case XmlDiffOperation.ChangeCharacterData:
                {
                    ...
                    switch (_targetNode.NodeType)
                    {
                        case XmlDiffNodeType.Text:
                        case XmlDiffNodeType.SignificantWhitespace:
                            xmlWriter.WriteAttributeString("type", "Content");
                            break;
                        case XmlDiffNodeType.Comment:
                            xmlWriter.WriteAttributeString("type", "Comment");
                            break;
                        case XmlDiffNodeType.CDATA:
                            xmlWriter.WriteAttributeString("type", "CDATA");
                            break;
                        default:
                            Debug.Assert(false);
                            break;
                    }
                    xmlWriter.WriteAttributeString("value", chd.Value);
                    xmlWriter.WriteEndElement();
                    break;
                }
            case XmlDiffOperation.ChangeER:
...
// Methods
    ...
    internal override void WriteToXmlReadable(XmlWriter xmlWriter, XmlDiff xmlDiff)
    {
        ...
        if(_sourceNode.OuterXml.IndexOf(">") +1 >= _sourceNode.OuterXml.Length)
            balise = _sourceNode.OuterXml.Substring(0, _sourceNode.OuterXml.IndexOf('/')) + '>';
        else
            balise = _sourceNode.OuterXml.Substring(0, _sourceNode.OuterXml.IndexOf('>') + 1);
