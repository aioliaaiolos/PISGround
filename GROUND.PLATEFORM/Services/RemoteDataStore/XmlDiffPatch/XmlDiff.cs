...
// Methods
    ...
    public bool CompareXmlReadable(string sourceFile, string changedFile, bool bFragments)
    {
        return CompareXmlReadable(sourceFile, changedFile, bFragments, null);
    }
    ...
    public bool CompareXmlReadable(string sourceFile, string changedFile, bool bFragments, XmlWriter diffgramWriter)
    {
        if (sourceFile == null)
            throw new ArgumentNullException("sourceFile");
        if (changedFile == null)
        ...
        try
        {
            ...
            return CompareXmlReadable(sourceReader, targetReader, diffgramWriter);
        }
        finally
        {
            if (sourceReader != null)
    ...
    public bool CompareXmlReadable(XmlReader sourceReader, XmlReader changedReader)
    {
        return CompareXmlReadable(sourceReader, changedReader, null);
    }
    ...
    public bool CompareXmlReadable(XmlReader sourceReader, XmlReader changedReader, XmlWriter diffgramWriter)
    {
        if (sourceReader == null)
            throw new ArgumentNullException("sourceReader");
        if (changedReader == null)
        ...
        try
        {
            ...
            // compare
            return DiffXmlReadable(diffgramWriter);
        }
        finally
        {
            _sourceDoc = null;
    ...
    public bool CompareXmlReadable(XmlNode sourceNode, XmlNode changedNode)
    {
        return CompareXmlReadable(sourceNode, changedNode, null);
    }
    ...
    public bool CompareXmlReadable(XmlNode sourceNode, XmlNode changedNode, XmlWriter diffgramWriter)
    {
        if (sourceNode == null)
            throw new ArgumentNullException("sourceNode");
        if (changedNode == null)
    ...
    private bool DiffXmlReadable(XmlWriter diffgramWriter)
    {
        try
        {
#if MEASURE_PERF 
...
#endif
            Debug.Assert(diffgramWriter != null);
            diffgram.WriteToXmlReadable(diffgramWriter);
            diffgramWriter.Flush();
#if MEASURE_PERF 
            _xmlDiffPerf._diffgramSaveTime = Environment.TickCount - startTickCount;
#endif
    ...
    private void MatchIdenticalSubtrees()
    {
                ...
                targetNodesToExpand.Count > 0 )
        {
            ...
            for ( int i = 0; i < count; i++ )
            {
                ...
                while ( curSourceNode != null )
                {
                    // it it'lEntryStream an attribute or the node has already been matched -> continue
