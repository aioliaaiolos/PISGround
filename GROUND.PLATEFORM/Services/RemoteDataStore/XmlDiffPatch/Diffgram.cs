...
// Methods
    ...
    {
        _xmlDiff = xmlDiff;
        WriteToXmlReadable(xmlWriter);
    }

    internal void WriteToXmlReadable(XmlWriter xmlWriter)
    {
        Debug.Assert(_xmlDiff._fragments != TriStateBool.DontKnown);
        xmlWriter.WriteStartDocument();
        xmlWriter.WriteStartElement("", "xmldiff", XmlDiff.NamespaceUri);
        WriteChildrenToXmlReadable(xmlWriter, _xmlDiff);
        xmlWriter.WriteEndElement();
        xmlWriter.WriteEndDocument();
    }
